...
LOGO_FILE = "dfu_logo.png"  # ✅ your logo filename in same folder as app.py
...
def find_total_row(agent_df: pd.DataFrame) -> Optional[pd.Series]:
    ...
    if "Day/date" in agent_df.columns:
        ...
        if len(candidates) > 0:
            st.success("✅ Method 1: Found TOTAL row where Day/date = '-'")
    ...
    if "Day/date" in agent_df.columns:
        ...
        if len(candidates) > 0:
            st.success("✅ Method 2: Found TOTAL row where Day/date is empty/NaN")
            return candidates.iloc[-1]
...
def extract_working_days(total_row: pd.Series) -> int:
    ...
    possible_columns = ["Days Work", "Days", "Working Days", "Work Days", "Days Worked", "Total Days"]
        ...
        if col in total_row.index:
            ...
            if pd.notna(val):
                try:
                    wd = int(float(val))
                    if wd > 0:
                        st.info(f"📊 Working Days found in column '{col}': {wd}")
                        return wd
                except:
                    pass
...
def load_logo_bytes() -> Optional[bytes]:
    """Loads dfu_logo.png automatically from same folder as app.py"""
    p = Path(LOGO_FILE)
...
def build_pdf_report(
    ...
    agent_meta_style = ParagraphStyle(
        "AgentMeta",
        parent=styles["Normal"],
        fontName="Helvetica",
        fontSize=10,
    ...
    def draw_background(canvas, doc):
        ...
        # White header area (top 4cm)
        canvas.setFillColor(colors.white)
        canvas.rect(0, A4[1] - 4.0*cm, A4[0], 4.0*cm, fill=1, stroke=0)

        # Red accent stripe at top
        canvas.setFillColor(DFU_RED)
        canvas.rect(0, A4[1] - 0.3*cm, A4[0], 0.3*cm, fill=1, stroke=0)
    ...
    if logo_bytes:
        try:
            ...
            # Scale logo
            target_width = 6.0 * cm
            target_height = (img_height / img_width) * target_width
            ...
            logo_img = Image(BytesIO(logo_bytes), width=target_width, height=target_height)
        except:
            logo_img = None

    # ✅ Header: logo on red tile to make white logo visible
    if logo_img:
        logo_tile = Table([[logo_img]])
        logo_tile.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, -1), DFU_RED),
        ...
        header_table = Table(
            [[logo_tile, Paragraph("Payslip Analysis Report", title_style)]],
    ...
    # ✅ Red divider line + space after it
    ...
    elements.append(Spacer(1, 12))  # ✅ little space after the line

    # ✅ Agent name bigger + bold
    elements.append(Paragraph(f"Agent: {agent_name}", agent_name_style))
    elements.append(Paragraph(f"Agent ID: {agent_id or 'N/A'} | Login ID: {login_id or 'N/A'}", agent_meta_style))
    elements.append(Paragraph(f"Period: {start_date} to {end_date} | Cutoff Hour: {cutoff_hour}:00 (Cairo)", agent_meta_style))
...
# =============================
# WEB HEADER (no markdown parsing issues)
# =============================
...
">
    <img src="data:image/png;base64,{logo_b64}" style="height:80px; width:auto; display:block;" />
</div>

        <div>
            <div style="font-size:38px; font-weight:700; line-height:1;">
                DFU-VA Payslip Analysis
            </div>
...
with st.expander("🔍 View Agent Data Structure"):
    ...
    st.dataframe(
        pay_agent[["Day/date", "Name", "Days Work", "Logged Time", "Payable (t)", "Break (t)"]].tail(5)
...
if total_row is not None:
    ...
    # Break is not payable
    payable_hours = max(0.0, payable_hours_raw - break_hours)

    st.info(
        f"✅ **Extracted:** {working_days} days, {format_num(payable_hours_raw)} raw hours - "
        f"{format_num(break_hours)} break = {format_num(payable_hours)} payable hours"
    )
else:
